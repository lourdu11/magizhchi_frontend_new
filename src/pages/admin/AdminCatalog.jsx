...
export default function AdminCatalog() {
  ...
  const catalogItems = useMemo(() => {
    ...
    // ── KEY FIX: Only show profiles that have at least 1 variant with PHYSICAL stock ──
    // If all inventory is deleted/archived OR has 0 totalStock, the product should not appear in the Catalog
    profiles = profiles.filter(p => {
      const items = finalMap[p._id] || [];
      if (items.length === 0) return false;
      
      const totalStock = items.reduce((sum, i) => sum + (i.totalStock || 0), 0);
      return totalStock > 0;
    });
                                      ...
                                      <div className="flex items-center gap-4">
                                         <button 
                                            ...
                                            onClick={() => {
                                               ...
                                               productService.updateProduct(product._id, { isActive: !product.isActive })
                                                  .then(() => {
                                                     queryClient.invalidateQueries(['admin-products']);
                                                     toast.success(`Web Status: ${!product.isActive ? 'ACTIVE' : 'INACTIVE'}`);
                                                  });
                                            }}
                                         ...
                                         <button 
                                            onClick={async () => {
                                               const targetStatus = !variants.some(v => v.offlineEnabled);
                                               toast.loading(`Updating POS status for ${variants.length} variants...`, { id: 'pos-sync' });
                                               try {
                                                  await Promise.all(variants.map(v => 
                                                     inventoryService.toggleChannel(v._id, { channel: 'offline' })
                                                  ));
                                                  queryClient.invalidateQueries(['admin-inventory']);
                                                  toast.success(`POS Status: ${targetStatus ? 'ACTIVE' : 'INACTIVE'}`, { id: 'pos-sync' });
                                               } catch (err) {
