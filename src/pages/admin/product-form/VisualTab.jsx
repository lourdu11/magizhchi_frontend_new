...
// ── Live Preview Panel ──────────────────────────────────────────
function LivePreview({ src, fit, position, scale, bgStyle, label, aspect }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 left-3 z-20 bg-black/50 backdrop-blur-sm text-white text-[8px] font-black px-2.5 py-1 rounded-full uppercase tracking-widest">
        {label}
      </div>
    </div>
  );
...
function PositionPicker({ value, onChange }) {
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="inline-grid grid-cols-3 gap-1.5 p-2 bg-light-bg rounded-2xl border border-border-light">
            ...
            <button
              ...
              className={`w-8 h-8 rounded-xl flex items-center justify-center transition-all
                ${current === pos
                  ...
                  : 'bg-white text-text-muted hover:bg-premium-gold/10 hover:text-premium-gold border border-border-light'
                }`}
            >
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        {[
          { val: 'contain', label: 'Full Show', desc: 'Show entire image' },
          { val: 'cover', label: 'Fill Frame', desc: 'Fill without gaps' },
        ].map(opt => (
          <button
            key={opt.val}
            type="button"
            onClick={() => onChange(opt.val)}
            className={`flex-1 p-3 rounded-2xl border text-left transition-all ${value === opt.val ? 'bg-charcoal text-white border-charcoal shadow-lg' : 'bg-white border-border-light hover:border-premium-gold'}`}
          >
  ...
  return (
    <div className="space-y-2">
      <div className="flex justify-between items-center">
        ...
        <span className="text-[9px] font-black text-charcoal">{Number(value || 1).toFixed(1)}×</span>
      </div>
      <div className="flex items-center gap-3">
        <button
          type="button"
          onClick={() => onChange(Math.max(0.5, Math.round((Number(value || 1) - 0.1) * 10) / 10))}
          className="w-9 h-9 rounded-xl bg-light-bg border border-border-light text-charcoal font-black flex items-center justify-center hover:bg-premium-gold hover:text-charcoal transition-all"
        >−</button>
        <div className="flex-1 h-2 bg-light-bg rounded-full relative border border-border-light overflow-hidden">
          <div
...
// ── Drop Zone ───────────────────────────────────────────────────
function DropZone({ onFile, loading, hasImage, label }) {
  ...
  return (
    <div
      ...
      className={`relative h-36 border-2 border-dashed rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all
        ${dragOver ? 'border-premium-gold bg-premium-gold/5 scale-[1.02]' : hasImage ? 'border-emerald-400 bg-emerald-50/30' : 'border-border-light bg-light-bg hover:border-premium-gold hover:bg-premium-gold/5'}`}
    >
      ...
      {loading ? (
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-[9px] font-black text-text-muted uppercase tracking-widest">Processing...</p>
        </div>
      ) : hasImage ? (
        <div className="flex flex-col items-center gap-2 text-emerald-600">
          <Check size={24} />
          <p className="text-[9px] font-black uppercase tracking-widest">Uploaded! Drop to replace</p>
        </div>
      ) : (
        <div className="flex flex-col items-center gap-2 text-text-muted">
          <Upload size={24} />
          <p className="text-[9px] font-black uppercase tracking-widest">Drop image or click to upload</p>
          <p className="text-[8px] font-bold opacity-60">{label}</p>
        </div>
      )}
    </div>
  );
